...
contract OnChainOptimized is IERC4883, ERC721F {
    ...
    function mint(uint256 numberOfTokens) external {
        ...
        while (numberOfTokens != 0 ) {
            uint256 algorithmId = last_selected + createRandomNumber(supply+1);
    ...
    function createRandomNumber(uint256 tokenId) public view returns (uint256) {
        uint256 random =  uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, tokenId, msg.sender))) % 32;
        return random;
    }
   ...
   /**
     ...
     */
    function getBackgroundId(uint256 Id) public pure returns (uint256 backgroundId){
        return Id % 6;
    }
     ...
     */
    function getBraceletId(uint256 Id) public pure returns (uint256 braceletId){
     ...
     */
    function getBracelet(uint256 bracelet_id) public pure returns(string memory){
    ...
    function getGlasses(uint256 glasses_id) public pure returns(string memory){
        string[4] memory glasses_colors = ['ed4949','ffff56', 'ff7f00', '00bf00'];
        string[4] memory big_glasses = ['<path stroke-width="4" fill="#',
        ...
        returns (string memory)
    {
        ...
        string memory output = string(abi.encodePacked(frame[0],getBackground(getBackgroundId(algorithmId)), frame[1], frame[2], getGlasses(getGlassesId(algorithmId))));
    ...
    function getTraits(
        ...
        uint256 algorithmId = IdToAlgorithmId[tokenId];
        require(_exists(tokenId), "Non-Existing token");
        string memory o=string(abi.encodePacked(tr1,Strings.toString(getBackgroundId(algorithmId)),tr2,Strings.toString(getBraceletId(algorithmId)),tr3,Strings.toString(getGlassesId(algorithmId))));
        return string(abi.encodePacked(o, tr4, Strings.toString(getPurseId(algorithmId)),tr5));
    }
