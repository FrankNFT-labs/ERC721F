    ...
    UUPSUpgradeable
{
    ...
    function _getColdWalletAddresses(
        ...
        WalletLink[] memory walletLinks = hotWalletToColdWallets[
            hotWalletAddress
        ];
    ...
    function _findColdWalletIndex(
        ...
        address[] memory coldWallets = _getColdWalletAddresses(
            hotWalletAddress
        );
    ...
    function _removeColdWalletFromHotWallet(
        ...
        uint256 coldWalletIndex = _findColdWalletIndex(
            coldWalletAddress,
            hotWalletAddress
        );
    ...
    function _addColdWalletToHotWallet(
        ...
        if (coldWalletIndex == NOT_FOUND) {
            hotWalletToColdWallets[hotWalletAddress].push(
                WalletLink(coldWalletAddress, expirationTimestamp)
            );
        }
    }
    ...
    function _setColdWalletToHotWallet(
        ...
        address currentHotWalletAddress = coldWalletToHotWallet[
            coldWalletAddress
        ].walletAddress;
        ...
        emit HotWalletChanged(
            coldWalletAddress,
            currentHotWalletAddress,
            hotWalletAddress,
            expirationTimestamp
        );
    }
    ...
    function setHotWallet(
        ...
        require(
            coldWalletToHotWallet[coldWalletAddress].walletAddress !=
                hotWalletAddress,
            "Already linked"
        );
         ...
         */
        _removeColdWalletFromHotWallet(
            coldWalletAddress,
            currentHotWalletAddress
        );
        if (hotWalletAddress != address(0)) {
            require(
                hotWalletToColdWallets[hotWalletAddress].length <
                    MAX_HOT_WALLET_COUNT,
                "Too many linked wallets"
            );

            _addColdWalletToHotWallet(
                coldWalletAddress,
                hotWalletAddress,
                expirationTimestamp
            );
    ...
    function setExpirationTimestamp(uint256 expirationTimestamp) external {
        address coldWalletAddress = _msgSender();
        address hotWalletAddress = coldWalletToHotWallet[coldWalletAddress]
            .walletAddress;
