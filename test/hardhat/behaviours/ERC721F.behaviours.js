...
import { loadFixture } from "../helpers/connection.js";

export default function (deployTokenFixture) {
    ...
    describe("walletOfOwner", function () {
        ...
        it("Should include the tokens minted by the sender", async function () {
            const walletOfOwner = await token.walletOfOwner(
                ownerAddress.address,
            );

            expect(walletOfOwner.map((t) => Number(t))).to.have.members([
                0, 1,
            ]);
        });
        ...
        it("Shouldn't assign minted tokens by another address to the owner of the contract", async function () {
            ...
            expect(walletOwner.map((t) => Number(t))).to.not.include.members(
                [2],
            );
            expect(walletOther.map((t) => Number(t))).to.have.members([2]);
        });
        ...
        it("Should jump the 0 address", async function () {
            ...
            expect(walletOwner.map((t) => Number(t))).to.have.members([1]);
        });
    });
