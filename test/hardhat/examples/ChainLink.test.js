...
describe("ChainLink", function () {
    async function deployTokenFixture() {
        ...
        await hardhatVrfMock.fundSubscription(1, ethers.parseEther("7"));

        const hardhatToken = await Token.deploy(1, hardhatVrfMock.target);

        await hardhatVrfMock.addConsumer(1, hardhatToken.target);
    ...
    describe("flipSaleState", function () {
        it("Reverts when startingIndex hasn't been set", async function () {
            ...
            await expect(hardhatToken.flipSaleState()).to.be.revertedWith(
        ...
        it("Coordinator should emit RandomWordsFulfilled when asking for random number", async function () {
            const { hardhatToken, hardhatVrfMock } =
            ...
            await expect(
                hardhatVrfMock.fulfillRandomWords(
                    requestId,
                    hardhatToken.target,
                ),
    ...
    describe("setRandomStartingIndex", function () {
        it("Should revert if the startingIndex has already been set", async function () {
            ...
            await expect(
                hardhatToken.setRandomStartingIndex(),
            ).to.be.revertedWith("startingIndex already set");
        });
    });

    describe("mint", function () {
        it("Should increase the totalSupply and walletOfOwner size", async function () {
            const { hardhatToken, hardhatVrfMock, owner } =
                await loadFixture(deployTokenFixture);
            ...
            expect(
                Object.keys(await hardhatToken.walletOfOwner(owner.address))
                    .length,
            ).to.be.equal(1);
        });
        ...
        it("Shouldn't assign non-owner mints to owner however should increase totalSupply", async function () {
            const { hardhatToken, hardhatVrfMock, owner, addr1 } =
                await loadFixture(deployTokenFixture);
            ...
            expect(
                Object.keys(await hardhatToken.walletOfOwner(addr1.address))
                    .length,
            ).to.be.equal(6);
        });
    });
...
async function retrieveRequestId(tx) {
    ...
    const [requestId] = logs.filter(
        (x) => x.eventName === "REQUESTEDRANDOMNESS",
    )[0].args;
