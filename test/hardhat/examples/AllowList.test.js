...
describe("AllowList", function () {
    ...
    context("mintPreSale", function () {
        describe("Inactive pre-sale", function () {
            it("shouldn't allow minting by whitelisted accounts during inactive pre-sale period", async function () {
                ...
                await expect(
                    hardhatToken.connect(addr1).mintPreSale(1, {
        ...
        describe("Active pre-sale", function () {
            ...
            it("shouldn't allow minting by unwhitelisted accounts during pre-sale period", async function () {
                await expect(
                    token.connect(nonWhitelistedAddress).mintPreSale(1, {
                        value: transferAmount,
                    })
                ).to.be.revertedWithCustomError(token, "AddressNotInAllowList");
            });
    ...
    context("mint", function () {
        describe("Inactive sale", function () {
            it("shouldn't allow minting by anyone", async function () {
                ...
                await expect(
                    hardhatToken.connect(addr6).mint(1, {
                        value: transferAmount,
                    })
                ).to.be.revertedWith("Sale NOT active yet");
            });
        ...
        describe("Active sale", function () {
            ...
            it("shouldn't allow minting by accounts which don't send enough funds", async function () {
                await expect(
                    token.connect(whitelistedAddress).mint(5, {
                        value: transferAmount,
                    })
                ).to.be.revertedWith("Ether value sent is not correct");
            });
    ...
    context("AllowList imported functions", function () {
        ...
        beforeEach(async () => {
            const { hardhatToken, addr1, addr6, addr7 } = await loadFixture(
                deployTokenFixture
            );
            token = hardhatToken;
            whitelistedAddress = addr1;
        ...
        describe("allowAddress", function () {
            it("should only be executable by the owner", async function () {
                await expect(
                    token.allowAddress(nonWhitelistedAddress.address)
                ).to.not.be.reverted;
                await expect(
                    token
                        .connect(nonWhitelistedAddress)
                        .allowAddress(nonWhitelistedAddress.address)
                ).to.be.reverted;
            });

            it("should add the address to the allowList", async function () {
                expect(
                    await token.isAllowList(nonWhitelistedAddress.address)
                ).to.be.false;
            ...
            it("shouldn't revert or disallow an existing allowed address", async function () {
                expect(
                    await token.isAllowList(whitelistedAddress.address)
                ).to.be.true;

                await expect(
                    token.allowAddress(whitelistedAddress.address)
                ).to.not.be.reverted;
        ...
        describe("allowAddresses", function () {
            it("should only be executable by owner", async function () {
                await expect(
                    token.allowAddresses([nonWhitelistedAddress.address])
                ).to.not.be.reverted;
                await expect(
                    token
                        .connect(nonWhitelistedAddress)
                        .allowAddresses([nonWhitelistedAddress.address])
                ).to.be.reverted;
            });
            ...
            it("should add the addresses to the allowList", async function () {
                ...
                expect(
                    await token.isAllowList(secondNonWhitelistedAddress.address)
                ).to.be.false;

                await token.allowAddresses([
                    nonWhitelistedAddress.address,
                    secondNonWhitelistedAddress.address,
                ]);
        ...
        describe("disallowAddress", function () {
            ...
            it("shouldn't revert or allow a non-existing allowed address", async function () {
                ...
                await expect(
                    token.disallowAddress(whitelistedAddress.address)
                ).to.not.be.reverted;
        ...
        describe("isAllowList", function () {
            it("should be executable by anyone", async function () {
                await expect(
                    token.isAllowList(whitelistedAddress.address)
                ).to.not.be.reverted;
                await expect(
                    token
                        .connect(nonWhitelistedAddress.address)
                        .isAllowList(whitelistedAddress.address)
                ).to.not.be.reverted;
            });
